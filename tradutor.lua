...
-- Gera arquivos de tradução manipulus.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por manipulus apartir de "..file.."\n# textdomain: manipulus\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/manipulus."..lang_code..".tr", "w")
...
manipulus.s = function(...)
	...
	minetest.log("error", "[Manipulus] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
...
manipulus.Sfake = function(s) return s end
